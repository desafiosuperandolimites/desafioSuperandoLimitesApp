...
class GestaoGrupoPage extends StatefulWidget {
  const GestaoGrupoPage({super.key});

  @override
  GestaoGrupoPageState createState() => GestaoGrupoPageState();
}

class GestaoGrupoPageState extends State<GestaoGrupoPage> {
  final GrupoController _grupoController = GrupoController();
  bool isAscending = false;
  List<Grupo> grupos = [];
  List<Grupo> filteredGrupos = [];
  String searchQuery = '';
  ...
  Future<void> _loadGruposFromBackend() async {
    await _grupoController.fetchGrupos(); // Fetch users from backend
    setState(() {
      grupos = _grupoController.groupList;
      filteredGrupos = List.from(grupos);
    });
  }

  void _filterGrupos() {
    setState(() {
      filteredGrupos = grupos.where((grupo) {
        return grupo.nome.toLowerCase().contains(searchQuery.toLowerCase());
      }).toList();
    });
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Campo de pesquisa e botão de adicionar grupo
          Padding(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => const CadastrarGrupoPage()),
                      );
                    },
                    ...
                    GestureDetector(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => CadastrarGrupoPage(
                              grupo: filteredGrupos[index],
                              isEditing: true,
                            ),
                          ),
                        );
