...
class StatusAprovacaoKMPage extends StatefulWidget {
  final Evento evento;
  final Usuario usuario;

  const StatusAprovacaoKMPage({
    super.key,
  ...
  @override
  StatusAprovacaoKMPageState createState() => StatusAprovacaoKMPageState();
}

class StatusAprovacaoKMPageState extends State<StatusAprovacaoKMPage> {
  ...
  void _sortDadosList() {
    setState(() {
      if (_isAscending) {
        _filteredDadosList
            .sort((a, b) => a.kmPercorrido.compareTo(b.kmPercorrido));
      } else {
        _filteredDadosList
            .sort((a, b) => b.kmPercorrido.compareTo(a.kmPercorrido));
      }
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            ...
            child: Center(
              child: Text(
                'Status de Aprovação de KM',
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 22 * screenWidth / 375,
              ...
              GestureDetector(
                onTap: () async {
                  bool? result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => DetalheStatusPage(
